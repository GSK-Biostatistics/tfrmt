...
apply_tfrmt <- function(.data, tfrmt, mock = FALSE) {
    ...
    tbl_dat_wide <- tbl_dat %>%
        ...
        tentative_process(
            arrange_enquo,
            tfrmt$sorting_cols,
            fail_desc = "Unable to arrange dataset"
        )

    if (is.null(tfrmt$row_grp_plan$label_loc)) {
        tfrmt$row_grp_plan$label_loc <- element_row_grp_loc(
            location = "indented"
        )
    }
    ...
    if (any(num_rec_by_row$n > 1)) {
        ...
        if (!mock) {
            suggested_frmt_structs <- num_rec_by_row %>%
                ...
                dplyr::mutate(label_quote = paste0('"', !!tfrmt$label, '"')) %>%
                dplyr::reframe(
...
frmt_struct_string <- function(grp, lbl, param_vals) {
    ...
    if (length(group_names) > 1) {
        group_val_char <- utils::capture.output(dput(stats::setNames(grp, group_names)))
    } else if (length(group_names) == 1) {
        group_val_char <- paste(utils::capture.output(dput(grp[[1]])), collapse = "")
    } else {
    ...
    if (is_empty(tfrmt$sorting_cols) == FALSE) {
        ...
        if (is_empty(tfrmt$group) == FALSE) {
            order_check <- .data %>%
                ...
                dplyr::mutate(
                    n1 = dplyr::n_distinct(!!(tfrmt$label), !!!tfrmt$sorting_cols),
