...
check_column_and_col_plan <- function(x) {
    ...
    if (span_structures_defined) {
        column_strings <- purrr::map_chr(x$column, rlang::as_label)
...
check_group_var_consistency_body_plan <- function(x) {
    if (!is.null(x$body_plan)) {
        ...
        group_as_char <- purrr::map_chr(x$group, rlang::as_label)
            ...
            if (is.list(struct$group_val)) {
                ...
                if (any(invalid_group_idx)) {
                    ...
                    is_invalid_body_plan_message <- c(
                        ...
                        paste0(
                            paste0(
                                "Invalid Format Structure in body_plan at position `",
                                idx,
                                "`:\n"
                            ),
                            paste0(
            ...
            if (is.list(struct$group_val)) {
                ...
                if (any(invalid_group_idx)) {
                    ...
                    is_invalid_row_grp_plan_message <- c(
                        ...
                        paste0(
                            paste0(
                                "Invalid Format Structure in row_grp_plan for row_grp_structure `",
                                idx,
                                "`:\n"
                            ),
                            paste0(
            ...
            if (is.list(struct$group_val)) {
                ...
                if (any(invalid_group_idx)) {
                    ...
                    is_invalid_footnote_plan_message <- c(
                        ...
                        paste0(
                            paste0(
                                "Invalid Format Structure in footnote_plan for footnote_structure `",
                                idx,
                                "`:\n"
                            ),
                            paste0(
...
check_col_style_row_grp_consistency <- function(x) {
    if (
        ...
        col_align_plan_as_char <- x$col_style_plan %>%
            lapply(function(x) x$cols %>% purrr::map_chr(rlang::as_label))
        for (cap_vars_idx in seq_along(col_align_plan_as_char)) {
            ...
            if (any(grp_in_cap[-1]) && r_grp_plan_col_loc != "column") {
                ...
                is_invalid_plan_message <- c(
                    ...
                    paste0(
                        paste0(
                            "Invalid col_style_structure in row_grp_plan at position `",
                            cap_vars_idx,
                            "`:\n"
                        ),
                        paste0(
