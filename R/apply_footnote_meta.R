...
apply_footnote_meta <- function(
    ...
    footnote_locs <- footnote_plan$struct_list %>%
        map(
            ...
            element_row_grp_loc = element_row_grp_loc,
            group = group,
            label = label,
            columns = columns
        )
...
get_col_loc <- function(footnote_structure, .data, col_plan_vars, columns) {
    ...
    if ("column_val" %in% names(loc_info)) {
        col_str <- columns %>% map_chr(rlang::as_label)
...
get_row_loc <- function(
    ...
    row_grp <- ifelse(
        is.null(element_row_grp_loc$location),
        "",
    ...
    if (any(names(loc_info) %in% c("group_val", "label_val"))) {
        ...
        if (!is.null(col_info$spanning) && col_info$spanning) {
            ...
            group_str <- group %>% map_chr(rlang::as_label)
