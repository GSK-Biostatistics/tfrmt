...
apply_col_alignment_pos <- function(col, align) {
    ...
    col_with_align <- col_with_align %>%
        ...
        dplyr::mutate(
            n_split_levs = stringr::str_count(
                .data$align,
                "(?<!\\\\)[\\|]"
            ) +
                1
    ...
    col_left_padded_sum <- col_left_padded01 %>%
        dplyr::group_by(.data$col_idx) %>%
        dplyr::summarise(col = paste0(.data$col_sub_out, collapse = "")) %>%
        dplyr::mutate(
...
apply_col_width <- function(col, width) {
    ...
    pad_left <- stringr::str_dup(
        " ",
        nchar(col) - nchar(trimws(col, "left"))
    )

    pad_right <- stringr::str_dup(
        " ",
