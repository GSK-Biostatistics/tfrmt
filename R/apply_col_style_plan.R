...
apply_col_style_plan <- function(
    ...
    for (el_idx in seq_along(style_plan)) {
        ...
        if (length(col_selections) > 0) {
            col_style_selection <- style_el[setdiff(
                names(style_el),
                "cols"
            )] %>%
                purrr::map(list) %>%
    ...
    if (length(total_col_style_selection) > 0) {
        ...
        for (col_style_idx in seq_len(nrow(total_col_styles))) {
            ...
            .data <- .data %>%
                dplyr::mutate(
                    !!col_to_modify := apply_style_to_col(
                        !!col_to_modify,
                        col_style_to_apply[setdiff(
                            names(col_style_to_apply),
                            "col"
                        )]
                    )
                )
...
apply_col_alignment_pos <- function(col, align) {
    ...
    col_with_align <- tibble::tibble(
        ...
        dplyr::left_join(
            tibble::tibble(
                align = trimws(align),
                col_as_x = stringr::str_replace_all(align, stringr::fixed("|"), "")
            ),
            by = "col_as_x"
        )
    ...
    col_with_align <- col_with_align %>%
        ...
        dplyr::mutate(
            n_split_levs = stringr::str_count(.data$align, "(?<!\\\\)[\\|]") + 1
        )
    ...
    col_with_pos <- col_with_align %>%
        ...
        dplyr::mutate(
            ...
            col_split_lev = gsub("col_split_", "", .data$col_split_lev, fixed = TRUE) %>%
                as.numeric()
        )
    ...
    col_with_splits <- col_with_pos %>%
        dplyr::mutate(
            ...
            col_sub_1 = dplyr::case_when(
                ...
                stringr::str_detect(.data$col_sub, stringr::fixed(" "), negate = TRUE) &
