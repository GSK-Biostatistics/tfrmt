...
create_stub_head <- function(
    ...
    if (length(grps) > 0 && length(col_plan_vars_chr) > 0) {
        ...
        nms_grps <- col_plan_grps[match(grps, col_plan_grps, nomatch = 0)] %>%
            names()
    ...
    if (is.null(cp)) {
        ...
        for (cp_el_idx in seq_along(cp$dots)) {
            ...
            col_selections <- col_selections[
                !duplicated(gsub("^-", "", col_selections), fromLast = TRUE)
            ]
        }
...
col_plan_quo_to_vars <- function(
    ...
    split_data_names <- split_data_names_to_df(
        data_names,
        preselected_cols,
        column_names
    )
    ...
    selected <- eval_col_plan_quo(
        x[[1]],
        data_names_tmp,
        preselected_cols,
        default_everything_behavior = default_everything_behavior
    )
    ...
    if (startsWith(rlang::as_label(x[[1]]), "-")) {
        ...
        if (!is.null(names(x)) && names(x) != "") {
            ...
            if (
                is_valid_tidyselect_call(rlang::quo_get_expr(x[[1]])) &&
                    length(selected) > 1
            ) {
...
col_plan_span_structure_to_vars <- function(
    ...
    ords <- do.call(tidyr::crossing, col_selections) %>%
        ...
        dplyr::mutate(
            ord_col = seq_len(dplyr::n())
        )

    split_data_names %>%
        dplyr::left_join(ords, by = names(col_selections)) %>%
