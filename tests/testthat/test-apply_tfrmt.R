...
test_that("Message is printed if order variables are causing mismatching rows",{
  ...
  expect_equal(
    safe_apply_tfrmt(data, tfrmt , mock = FALSE)$messages[2],
    "Note: Some row labels have values printed over more than 1 line. \n")
  expect_equal(
    safe_apply_tfrmt(data, tfrmt , mock = FALSE)$messages[3],
  ...
  expect_equal(
    safe_apply_tfrmt(data2, tfrmt2 , mock = FALSE)$messages[2],
    "Note: Some row labels have values printed over more than 1 line. \n")
  expect_equal(
    safe_apply_tfrmt(data2, tfrmt2 , mock = FALSE)$messages[3],
    "This could be due to incorrect order variables. Each row in your output table should have only one order var combination assigned to it.\n")
