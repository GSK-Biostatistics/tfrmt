...
test_that("col_style_structure - advanced", {
    ...
    element_4 <- col_style_structure(
        align = ".",
        col = c(trt1, span_structure(col = test, col2 = value))
    )
    ...
    expect_equal(
        element_3$cols,
        list(list(col = vars(test), col1 = vars(value))),
        ignore_attr = TRUE
    )
    expect_equal(
        element_4$cols,
...
test_that("multi-positional alignment", {
    ...
    vec_aligned <- apply_col_alignment(
        vec,
        align = c(" xx.x |(xx.x)", " xx, |xx", "xx |(xx - xx)", " xx |(xx %)"),
        type = "pos"
    )
    ...
    vec_aligned <- apply_col_alignment(
        vec,
        align = c(" xx|.x", " x|, |x", "xx| |(xx - |xx)", " x| |(x.x, |x.x)"),
        type = "pos"
    )
