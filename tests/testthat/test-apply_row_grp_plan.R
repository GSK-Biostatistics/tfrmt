...
test_that("insert post space after specific value", {
    df <- tibble::tibble(
        tidyr::crossing(grp1 = c("A", "B", "C"), grp2 = c("a", "b")),
        label = "1",
        trtA = rep("xx (xx%)", 6),
        trtB = rep("xx (xx%)", 6),
        trtC = rep("xx (xx%)", 6),
...
test_that("Check combine_group_cols with a single group", {
    mock_single_grp <- tibble::tibble(
        tidyr::crossing(grp1 = c("A", "B", "C"), lab = c("a", "b")),
        trtA = rep("xx (xx%)", 6),
        trtB = rep("xx (xx%)", 6),
        trtC = rep("xx (xx%)", 6),
    )
...
test_that("row order is retained for all selections", {
    ...
    tfrmt(
        ...
        print_to_gt(dat %>% dplyr::select(-ord))
...
test_that("Check apply_row_grp_lbl - expect error when NA in label column", {
    ...
    mock_multi_grp$my_label <- ifelse(
        mock_multi_grp$my_label == "my_label_1",
        NA,
        mock_multi_grp$my_label
    )
    ...
    expect_error(
        ...
        paste(
            "`label` column my_label contains NA values. For group-level summary data, `label` and the relevant `group` values should match."
        )
    )
})
