...
test_that("Simple Case big_n", {
  ...
  auto <- apply_tfrmt(.data = data, tfrmt = tfrmt_auto, mock = FALSE) %>%
    names()
  ...
  auto_mock <- apply_tfrmt(.data = select(data, -Value), tfrmt = tfrmt_auto, mock = TRUE) %>%
    names()
  man_mock <-  big_ns %>%
    mutate(foo = str_c(Column, "\nN = xx")) %>%
    pull(foo) %>%
    c("Label", . , "..tfrmt_row_grp_lbl")
  expect_equal(auto_mock, man_mock)
