test_that("shuffle/trim works", {
    ...
    ard_simple <- cards::ard_continuous(
        cards::ADSL,
        by = "ARM",
        variables = "AGE"
    )
    ...
    expect_true(!any(is.na(ard_grp_shuffled$ARM)))
    ...
    ard_hier_shuff <- ard_hier |>
        ...
        as.data.frame()
    expect_true(!any(is.na(ard_hier_shuff$AESOC)))
    ...
    expect_snapshot(
        cards::bind_ard(
            cards::ard_categorical(
                ...
                dplyr::slice(1),
            cards::ard_categorical(cards::ADSL, by = SEX, variables = AGEGR1) |>
                dplyr::slice(1),
            cards::ard_categorical(cards::ADSL, variables = AGEGR1) |>
                dplyr::slice(1)
        ) |>
...
test_that("shuffle_card() fills grouping columns with `Overall <var>` or `Any <var>`", {
    ...
    shuffled_ard_hier <- ard_hier |>
        shuffle_card(
            fill_overall = "overall {colname} observed",
...
test_that("shuffle_card() prioritizes supplied `by` and messages on mismatch", {
    ...
    ard_mismatch <- cards::ard_continuous(
        cards::ADSL,
        by = "SEX",
        variables = "AGE"
    )
    ...
    ard_match <- cards::ard_continuous(
        cards::ADSL,
        by = "ARM",
        variables = "AGE"
    )
