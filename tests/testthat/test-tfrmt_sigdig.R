...
test_that("build frmt objects", {
    ...
    expect_equal(
        frmt_spec,
        list(frmt('xx.x', missing = '--'), frmt('xx.xx', missing = '--'))
    )

    # frmt_combine
    frmt_spec <- frmt_combine_builder(
        param_combine = "{mean} ({sd})",
        param = c("mean", "sd"),
        frmt_string = c("xx.x", "xx.xx"),
        missing = "-"
    )
    frmt_string <- list(frmt_combine(
        '{mean} ({sd})',
        mean = frmt('xx.x', missing = '-'),
        sd = frmt('xx.xx', missing = '-'),
        missing = '-'
    ...
    bp_1grp_1lbl_man <- list(
        frmt_structure(
            ...
            label_val = c("ALANINE AMINOTRANSFERASE", "CHOLESTEROL"),
            min = frmt('x.xx')
        ),
        frmt_structure(
            group_val = list(group1 = "CHEM"),
    ...
    bp_prm <- body_plan_builder(
        ...
        group = vars(group1),
        label = quo(group2),
        param_defaults = param_set(max = 0, "{pct}%" = 0)
    )
    bp_prm_man <- list(
        frmt_structure(
            group_val = list(group1 = "CHEM"),
...
test_that("no redundant frmt_structures", {
    ...
    bp_man <- body_plan(
        ...
        frmt_structure(
            group_val = list(group1 = "CHEM"),
            label_val = c("ALANINE AMINOTRANSFERASE", "BILIRUBIN"),
            min = frmt('x.xx')
        ),
        frmt_structure(
            group_val = list(group1 = "CHEM"),
        ...
        frmt_structure(
            group_val = list(group1 = c("CHEM", "HEM")),
            label_val = c("CHOLESTEROL", "EOSINOPHILS"),
            min = frmt('x.xxx')
        ),
        frmt_structure(
            group_val = list(group1 = c("CHEM", "HEM")),
    ...
    bp_man <- body_plan(
        frmt_structure(
            group_val = list(group1 = "CHEM", group2 = ".default"),
            label_val = c("v2"),
            min = frmt('x.xx')
        ),
        frmt_structure(
        ...
        frmt_structure(
            group_val = list(group1 = ".default", group2 = c("EOSINOPHILS")),
            label_val = c("v1"),
            min = frmt('x.xxx')
        ),
        frmt_structure(
        ...
        frmt_structure(
            group_val = list(group1 = "CHEM", group2 = "BILIRUBIN"),
            label_val = c("v1"),
            min = frmt('x.xx')
        ),
        frmt_structure(
        ...
        frmt_structure(
            group_val = list(group1 = "HEM", group2 = "CHOLESTEROL"),
            label_val = c("v1"),
            min = frmt('x.xxx')
        ),
        frmt_structure(
...
test_that("tfrmt_sigdig returns a tfrmt", {
    ...
    expect_equal(length(t_frmt$body_plan), length(param_set()) + 1)
})
...
test_that("varying group/label inputs", {
    ...
    expect_warning(
        ...
        paste0(
            "`sigdig_df` input does not contain the following group params: grp3"
        )
    )
...
test_that("group vars specified in tfrmt but not sigdig data are represented in body_plan", {
    ...
    expect_warning(
        ...
        paste0(
            "`sigdig_df` input does not contain the following group params: newgrp"
        )
    )

    bp_man <- body_plan(
        frmt_structure(
            group_val = list(group1 = "test1", newgrp = ".default"),
            label_val = c(".default"),
            min = frmt('x.xx')
        ),
        frmt_structure(
...
test_that("tfrmt_sigdig can be layered onto another tfrmt", {
    ...
    new_tfrmt <- tfrmt_sigdig(dat_sigdig, tfrmt_obj = prev_tfrmt)
    ...
    bp_man <- body_plan(
        ...
        frmt_structure(
            group_val = list(group1 = "test1", group2 = "test2"),
            label_val = c(".default"),
            min = frmt('x.xx')
        ),
        frmt_structure(
    ...
    prev_tfrmt <- tfrmt(group = somegrp, label = group2)
    dat_sigdig <- tibble::tribble(
        ~group1 , ~group2 , ~sigdig ,
        "test1" , "test2" ,       1
    )
