...
test_that("Test big n with footnotes", {
  ...
  expect_equal(
    big_n_footnote_plan_gt$`_footnotes` |>
      dplyr::select(locname, colname, locnum, rownum, footnotes) |>
      tibble::as_tibble(),
    tibble::tibble(
      locname = c(
        "columns_columns",
...
test_that("big Ns constant by page", {
  ...
  # big Ns constant by page but by_page=TRUE in the big_n_structure gives
  # message
  mytfrmt <- tfrmt(
    group = Group,
    label = Label,
