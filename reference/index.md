# Package index

## Build tfrmt

- [`tfrmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/tfrmt.md)
  : Table Format
- [`body_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/body_plan.md)
  : Table Body Plan
- [`frmt_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_structure.md)
  : Format Structure Object
- [`frmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt.md)
  [`frmt_combine()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt.md)
  [`frmt_when()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt.md)
  : Table Value Formatting
- [`col_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/col_plan.md)
  [`span_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/col_plan.md)
  : Define the Column Plan & Span Structures
- [`col_style_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/col_style_plan.md)
  : Column Style Plan
- [`col_style_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/theme_element.md)
  : Column Style Structure
- [`row_grp_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/row_grp_plan.md)
  : Row Group Plan
- [`row_grp_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/row_grp_structure.md)
  : Row Group Structure Object
- [`element_block()`](https://gsk-biostatistics.github.io/tfrmt/reference/element_block.md)
  : Element block
- [`element_row_grp_loc()`](https://gsk-biostatistics.github.io/tfrmt/reference/element_row_grp_loc.md)
  : Element Row Group Location
- [`footnote_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/footnote_plan.md)
  : Footnote Plan
- [`footnote_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/footnote_structure.md)
  : Footnote Structure
- [`big_n_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/big_n_structure.md)
  : Big N Structure
- [`page_plan()`](https://gsk-biostatistics.github.io/tfrmt/reference/page_plan.md)
  : Page Plan
- [`page_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/page_structure.md)
  : Page structure

## Layer

- [`layer_tfrmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/layer_tfrmt.md)
  : Layer tfrmt objects together
- [`update_group()`](https://gsk-biostatistics.github.io/tfrmt/reference/update_group.md)
  : Remap group values in a tfrmt

## Print/Export

- [`print_mock_gt()`](https://gsk-biostatistics.github.io/tfrmt/reference/print_mock_gt.md)
  : Print mock table to GT
- [`print_to_gt()`](https://gsk-biostatistics.github.io/tfrmt/reference/print_to_gt.md)
  : Print to gt
- [`print_to_ggplot()`](https://gsk-biostatistics.github.io/tfrmt/reference/print_to_ggplot.md)
  : Print to ggplot
- [`tfrmt_to_json()`](https://gsk-biostatistics.github.io/tfrmt/reference/tfrmt_to_json.md)
  : Print to JSON
- [`json_to_tfrmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/json_to_tfrmt.md)
  : json to tfrmt
- [`as_json()`](https://gsk-biostatistics.github.io/tfrmt/reference/as_json.md)
  : Convert tfrmt elements

## Included Templates

- [`tfrmt_n_pct()`](https://gsk-biostatistics.github.io/tfrmt/reference/tfrmt_n_pct.md)
  : N Percent Template
- [`tfrmt_sigdig()`](https://gsk-biostatistics.github.io/tfrmt/reference/tfrmt_sigdig.md)
  : Create tfrmt object from significant digits spec
- [`param_set()`](https://gsk-biostatistics.github.io/tfrmt/reference/param_set.md)
  : Set custom parameter-level significant digits rounding

## Example Data

- [`data_ae`](https://gsk-biostatistics.github.io/tfrmt/reference/data_ae.md)
  : Adverse Events Analysis Results Data
- [`data_demog`](https://gsk-biostatistics.github.io/tfrmt/reference/data_demog.md)
  : Demography Analysis Results Data
- [`data_efficacy`](https://gsk-biostatistics.github.io/tfrmt/reference/data_efficacy.md)
  : Efficacy Analysis Results Data
- [`data_labs`](https://gsk-biostatistics.github.io/tfrmt/reference/data_labs.md)
  : Labs Analysis Results Data

## Helpers

- [`display_row_frmts()`](https://gsk-biostatistics.github.io/tfrmt/reference/display_row_frmts.md)
  : Display formatting applied to each row
- [`display_val_frmts()`](https://gsk-biostatistics.github.io/tfrmt/reference/display_val_frmts.md)
  : Display formatted values
- [`apply_frmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/apply_frmt.md)
  : Apply formatting
- [`cleaned_data_to_gt()`](https://gsk-biostatistics.github.io/tfrmt/reference/cleaned_data_to_gt.md)
  : Do all the formatting for the GT
- [`is_frmt()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_utils.md)
  [`is_frmt_combine()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_utils.md)
  [`is_frmt_when()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_utils.md)
  [`is_frmt_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_utils.md)
  [`is_row_grp_structure()`](https://gsk-biostatistics.github.io/tfrmt/reference/frmt_utils.md)
  : Check if input is a frmt
- [`make_mock_data()`](https://gsk-biostatistics.github.io/tfrmt/reference/make_mock_data.md)
  : Make mock data for display shells

## ARD Helpers for {cards} to {tfrmt}

- [`shuffle_card()`](https://gsk-biostatistics.github.io/tfrmt/reference/shuffle_card.md)
  **\[experimental\]** :

  Shuffle `cards`

- [`prep_big_n()`](https://gsk-biostatistics.github.io/tfrmt/reference/prep_big_n.md)
  **\[experimental\]** :

  Prepare `bigN` stat variables

- [`prep_combine_vars()`](https://gsk-biostatistics.github.io/tfrmt/reference/prep_combine_vars.md)
  **\[experimental\]** : Combine variables

- [`prep_hierarchical_fill()`](https://gsk-biostatistics.github.io/tfrmt/reference/prep_hierarchical_fill.md)
  **\[experimental\]** : Fill missing values in hierarchical variables

- [`prep_label()`](https://gsk-biostatistics.github.io/tfrmt/reference/prep_label.md)
  **\[experimental\]** : Prepare label
