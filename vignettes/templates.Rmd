...
`tfrmt` offers the function `layer_tfrmt()`, which provides the ability for layering `tfrmt` together. The first two arguments are the `tfrmt` objects to be layered. By default the body_plans of the tfrmt are joined together.
...
In addition to defining full tables, frmts can also be defined at an organizational level. The benefit of this is that teams can use these frmt's in their tfrmt directly and can be sure their table will comply with the value presentation expected of their organization.
