...
data <- tibble::tibble(Group = rep(c("Age (y)", "Sex", "Age (y)", "Sex"), c(3, 3, 6,12)),
  ...
  dplyr::mutate(Value = dplyr::case_when(Param == "pct" ~ Value * 100,
                           TRUE ~ Value),
...
```{r}
big_ns <- tibble::tibble(Column = c("Placebo", "Treatment", "Total"),
                 Param = "bigN",
                 Value = c(30,30,60))
...
big_ns <- tibble::tibble(col = c("Placebo","Placebo","Treatment","Treatment"),
                 grp = c("A", "B", "A", "B"),
                 param = "bigN",
                 val = c(34, 36, 42, 39))
