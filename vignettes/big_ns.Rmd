...
## Page-Level Big Ns
If our table has a `page_plan` and is to be split by every unique value of a grouping variable, we may desire to have different big Ns for each split of the table. In this case, a user can supply group-level big Ns to the data. In order for this to work properly group levels MUST align to those indicated in the `page_plan` (i.e. any group or label values set to ".default" in the `page_structure`). This functionality can be enabled via the `by_page` argument in `big_n_structure`.

For example, let's take the following data with group-level bigNs calculated:
```{r}
dat <- crossing(
  grp = c("A","B"),
  lbl = c("a","b"),
...
Now we can make our paginated `tfrmt` and set `by_page` to `TRUE`:
```{r}
gts <- tfrmt(
  group = grp,
  label = lbl,
