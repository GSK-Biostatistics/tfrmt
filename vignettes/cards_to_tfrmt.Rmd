...
{tfrmt} contains several helper functions to ease the transition from the `cards` output to a display-ready ARD fit for {tfrmt}. Below are two examples utilitizing these helpers for a demographics and AE table. 
...
For our {tfrmt} table, `AGEGR1` and `AGE` need to be combined into a single variable. We can use the `prep_unite_vars()` helper for this:

```{r}
ard_demog_shuffled <- ard_demog_shuffled |> 
  prep_unite_vars(vars = c("AGEGR1","AGE"))
...
tfrmt(
  ...
  body_plan = body_plan(
    ...
    frmt_structure(group_val = ".default", label_val = ".default", p = frmt("xx%", transform = ~.*100))
  ),
...
We need to populate these missing values for our table. In order to display these as group summaries, the value for AEDECOD must match the value for AESOC. We can use the `prep_fill_pairwise` helper to fill the value from AESOC into AEDECOD:

```{r}
ard_ae_shuffled <- ard_ae_shuffled |> 
  prep_fill_pairwise(vars = c("AESOC","AEDECOD"), fill_from = "left")
