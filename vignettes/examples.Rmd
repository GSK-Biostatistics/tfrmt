...
*NOTE:* Pay attention that `label` in the `data_efficacy` must be matching the `label_val` in the `label_val` for the `frmt_combine()` to work. For example, for `Mean (SD)`, each variable has different
