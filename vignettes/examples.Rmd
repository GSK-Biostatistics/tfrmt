...
Now this table looks just about right. There are two problems, (1) alignment and (2) spacing between the continuous and categorical values. To take care of the alignment we are going to add a `col_style_plan` which accepts a series of `element_style`s. This allows columns to be aligned differently if needed. For this table, we want all the columns to align on either ".", "," or " " so our `element_style` looks like `element_style(align = c(".",","," "), col = vars(everything()))`. 
...
tfrmt(
  ...
  # Specify column styling plan
  col_style_plan = col_style_plan(
    element_style(align = c(".",","," "), col = vars(starts_with("p_")))
  )
  ) %>% 
  print_to_gt(ae_data2) %>% 
  tab_options(
...
tfrmt(
  ...
  # Specify column styling plan
  col_style_plan = col_style_plan(
    element_style(align = c(".",","," "), col = vars(contains("pval")))
  ),
...
```{r }
ae_data3 <- ae_data2 %>% 
  unite("combined_col", c(col2, col1), sep = "-")
...
For this example, we will use the `efficacy_data` dataset, an example analysis results dataset found in the package, which is based on the CDISC pilot data for the ADAS-Cog(11) score. The goal is to recreate table 14-3.01 from the CDISC pilot. 
...
tfrmt(
  ...
  col_plan = col_plan(
    group, label, Placebo, contains("Low"), contains("High")
  )
) %>%
  print_to_gt(efficacy_data) %>% 
  tab_options(
