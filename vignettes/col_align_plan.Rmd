...
tlang offers the ability to align value columns in the following ways:

- Left-align
- Right-align
- Align on one or more characters (e.g., decimal alignment)

Alignment is specified via the `col_align_plan`, which accepts a series of `element_align`s as inputs. Note that the `col_align_plan` does not have any `*_structure` objects as inputs, as `*_structure`s are for modifying the *contents* (i.e., text) of the table, while `element_*`s are for modifying *aesthetics*. 

Alignment is achieved by padding the values with character spaces; this padding ensures that alignment is robust and agnostic to the output format. 

## Alignment examples

Element aligns have two arguments: the alignment desired and the columns in the final table to be aligned. The column values that are specified are to match the contents of the `column` variable that is specified in the `tfrmt` object. 
...
Column alignment can improve this table by making it easier to read. Let's start by left-aligning our p-value column. Notice that by providing a `col_align_plan`, any variables that are not covered in the plan will be left-aligned by default.
...
tfrmt(
  ...
  col_align =  col_align_plan(
    element_align(align = "left", col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_align =  col_align_plan(
    element_align(align = c("."), col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_align =  col_align_plan(
    element_align(align = c("."), col = `p-value`),
    element_align(align = "left", col = trt1),
    element_align(align = "right", col = trt2))
  ) %>% 
  print_to_gt(dat)
```
