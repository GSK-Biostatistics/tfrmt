...
In the case of a single column variable with no column spanners, `col_plan` behaves identically to `dplyr::select`. 
...
## Column spanners
 ...
 The `col_plan` accepts a combination of tidyselect expressions, column names, and/or span structure (`span_structure`) objects. Recall that `*_structure` objects are for modifying the *contents* (i.e., text) of the table; in the `col_plan`, we use `span_structure`s to add column spanners. These are placed strategically in the `col_plan` to inform `tlang` of the hierarchy of spanners and columns.
 
A `span_structure` is constructed from a label which defines the spanner text, and a combination of tidyselect expressions and column names.
 
Let's try putting a spanner over the columns starting with "col":
...
We can also do any number of hierarchical spanners:
...
Finally, we can do renaming as well:
...
# Option 2: >1 `column` variable

Multiple `column` variables are used to form the hierarchy of column spanners and column labels, which is driven by the order of the `column` variables. The first variable specified represents the highest level spanner, while the last variable specified represents the lowest level column label. Similar to the case of 1 `column` variable, the user may remove and rename values for the lowest level `column` variable. With multiple `column` variables, however, specifying column ordering in the `col_plan` is a bit trickier. By default, `tlang` will display the columns as they appear in the data but these can be customized in the `col_plan`. The main limitation with >1 `column` variable is the inability to specify column spanners in the `col_plan`; the spanning labels will be created using the higher level `column` variable values, which will be displayed as-is. As such, there is a higher importance for preparing the analysis results data for display. 
...
When there are multiple `column` variables provided, `tlang` concatenates them behind the scenes using a special delimiter. Therefore, the underlying column names in the final table are a combination of the spanner(s) and the lowest level column label. In order to change the order of spanning groups, we may use`contains()` to reference any portion of those names. 

Reordering with multiple `column` variables is simplest when the lowest level column variable contains unique values (i.e., there is 1 value per column in the final table), like in the example above. Let's suppose we want to move the "col 4" column to the beginning and also reorder "col1" and "col2". To achieve this, we can just select "col 4" and then "col2" and finally all the other columns. 
```{r}
tfrmt(
    group = group,
    label = label,
...
In summary, the use of 1 or >1 `column` variables is a personal choice. Depending on the chosen workflow, a user can achieve the desired column ordering and labeling through a combination of data preparation and `tfrmt` options.
