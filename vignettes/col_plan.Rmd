...
For this table we want to ensure `"Treatment"` is always before `"Placebo"` and that `"mean"` comes before `"sd"`. To do that we can make one `span_structure` that specifices both the order of the `span` level and the order of the `my_col`  level. 
