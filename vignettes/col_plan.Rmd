...
## Editing column spanners

 The `col_plan` accepts a combination of tidyselect expressions, column names, and/or span structure (`span_structure`) objects. Recall that `*_structure` objects are for modifying the *contents* (i.e., text) of the table; in the `col_plan`, we use `span_structure`s to edit and reorder column spanners more specifically. These are placed strategically in the `col_plan` to inform `tfrmt` of the hierarchy of spanners and columns.
 
A `span_structure` is constructed from a collection of named arguments, where the name of the argument is the column variable and then the values entered are the values in that column, represented by a combination of tidyselect expressions and column names.
 
Let's try putting a moving the spanner over the columns starting with "col":
