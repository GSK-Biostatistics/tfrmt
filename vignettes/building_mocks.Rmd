...
values. Note that using the lower or upper case is up to your preference.
...
You can also use `span_structure()` to add spanning labels as below.

Note: You might see a warning along the lines of 
...
Alternatively you can add a span column to your dataset and allocate names for 
each column. Subsequently, ensure you concatenate your `span` and `column` 
within `c()` as arguments for the `column` parameter in `tfrmt()`.

For more information on this please refer to the "Column Plan" article.

```{r}
span_df <- crossing(label = c("label 1", "label 2", "label 3"),
                    column = c("PL", "T1", "T2"),
                    param = c("count", "percent")) %>%
...
Note that if you'd like to print a select few columns from your dataset you 
can simply filter `df` as normal within the `print_mock_gt()` function.

Here we have removed any row values that start with "Risk Diff" using 
regex in `str_detect`. You may choose to filter differently; for example, 
`df %>% filter(column != "PL")` would removed rows that match "PL".
...
tfrmt(
  ...
  print_mock_gt(df %>% filter(!stringr::str_detect(column, "^Risk Diff")))
```

### Same value formatting but 1 grouping level

In this example you can see that the dataset contains one group variable (inside 
`crossing()`). Hence, we account for this by adding the name of the group column 
into the `group` parameter for `tfrmt()`.
...
row label. Given that each label possess different parameter values in this 
...
generic "label #" names - this is just an illustration of what these row labels 
could be.
...
level. Thus, we have two instances of `crossing()` with different 
grp1-param combinations.
