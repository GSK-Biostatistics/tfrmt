...
representative of your final ARD, except without the `values` column) </li>
</ol>
...
it is a reminder to include a `values` parameter to the tfrmt specification 
...
tfrmt_spec <- tfrmt(
  ...
  body_plan = body_plan(
    frmt_structure(group_val = ".default", label_val = ".default",
                   n_pct_frmt(n = "count", pct = "percent") )),
...
each column. Subsequently, ensure you include both your `span` and `column` 
...
To add a title, subtitle or footer pass arguments to their respective parameters.

Note that the `n_pct_frmt()` function has been used in place of `frmt_combine()`. 
This function returns the same the formatting as the previous example, please 
