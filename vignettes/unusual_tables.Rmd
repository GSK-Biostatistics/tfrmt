...
Once we have the data in the standard ARD format we can make the tfrmt. What makes this tfrmt different is we won't include group or label, and our two ID columns will be displayed as regular columns. This also means that all columns of the table, including the ID columns, can be ordered via the `col_plan()`. Because the `col_plan()` follows the conventions of `select()` we are going to start with the lowest level columns, and then select the order of the spanning columns. 
```{r}
tfrmt(
  param = "param",
  values = "val",
