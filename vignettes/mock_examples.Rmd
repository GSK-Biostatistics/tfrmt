...
```{r}
df1 <- crossing(label = c("label 1", "label 2", "label 3"),
           column = c("T1", "T2", "T1&T2", "PL"),
           param = c("count", "percent")) 
df11 <- df1 |>
    mutate(ord1 = rep(seq(1:length(unique(df1$label))), each = nrow(df1)/length(unique(df1$label)) ))
  
  
df2 <- crossing(label = c("label 1", "label 2", "label 3"),
           column = c("risk T1-PL", "risk T2-PL", "risk T1&T2-PL"),
           param = c("num", "lower", "upper")) 
df22<- df2 |>
    mutate(ord1 = rep(seq(1:length(unique(df2$label))), each = nrow(df2)/length(unique(df2$label)) ))

df <- bind_rows(df11, df22)  |>
  arrange_all()
