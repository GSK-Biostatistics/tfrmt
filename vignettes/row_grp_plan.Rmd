...
-   **spanning:** Top-level group label as row spanning header, remaining group/label columns indented
...
In this case, group and row labels become separate columns on the left (forming the table "stub" in {gt}). 
*Note: in {tfrmt} 0.2.1 and earlier, only a maximum of 2 grouping columns was possible. If multiple grouping variables were present, the top level group variable appeared in the left-most column, and the remaining grouping variables were combined with the label variable using indentation.*
