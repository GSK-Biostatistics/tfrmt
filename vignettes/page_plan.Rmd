...
If any of the `page_structure`s contain a `".default"`, it may be desired to print a note indicating the grouping value for the given page at the time of rendering. The location of this note can be specified via the `note_loc` parameter in `page_plan`. The functionality for `note_loc` may be limited by the desired output type; for example, "preheader" is only available for RTF outputs, while "source_note" and "subtitle" are available for all output types. The content of this note can be modified by supplying a function via the `transform` argument.
...
Note can sometime not be very human readable. If we want to change the note (let's say from `"rowlbl1: Age (y), grp: cat"` to `"Row label 1 - Age (y), Group - cat"`), we can do that by supplying a transformation function to `page_plan()` - i.e. the `transform` argument.
