...
As two tfrmt are layered, it is possible for some of them to have conflicting information for which groups to use across their body_plans since this information is concatenated. Addressing this without having to re-write the entire body_plan is able to be done with the `update_group()` function. 
...
## provided data had different colnames for groups
alternate_labs_data <- labs_data_subset %>% 
  rename(
    `Lab Type` = group1,
    `Lab Test` = group1,
  )
