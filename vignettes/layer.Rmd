...
A core design element of `tlang` is to offer the ability to layer and build template table formats that can be shared, improved, and eventually customized for a specific table. The idea being that organizations can develop a standard `tfrmt` object that can be reused across many different tables, and make it so programmers can focus on setting up the data as opposed to formatting of the table. 

There are two ways that `tfrmt` objects may be layered. The preferred method is through the first argument of `tfrmt` - `tfrmt_obj`. The second, but perhaps more explicit method is using the `layer_tfrmt` function. We will go through their applications below.  

## Creating a template

Before we show how to layer a tfrmt, we need to create a template tfrmt. A template is intended to provide the conventions that an organization would expect to be repeated across multiple tables. For example, in a table such as an Adverse Events table, there are some standards that can be expected that allows us to constructing a basic `tfrmt`:
...
Now we have a template to serve as the basis for our AE tables, which reduces much of the overhead in our formatting. We can continue to build on this template by making a study-specific `tfrmt` to be layered on top. This can be done a few ways. The first method that can be used is to pipe another tfrmt. The first argument of `trfmt` is "tfrmt_obj". This allows for `tfrmt` to be layered directly using a pipe. 

We will create the new tfrmt with specific information that we know we have for this study, such as the columns indicating order (ord1, ord2), formatting of the row group labels, column alignment, and column labels.
...
The previous example of a template `tfrmt` is very powerful in that it allows components to be reused across multiple projects and share the learning from prior work. However, the downside is that we are limited to 2 layers. Our template `tfrmt` must be specified first, followed by our study-specific format.
