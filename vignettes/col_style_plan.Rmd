...
Alignment and width is specified via the `col_style_plan`, which accepts a series of `element_style`s as inputs. Note that the `col_style_plan` does not have any `*_structure` objects as inputs, as `*_structure`s are for modifying the *contents* (i.e., text) of the table, while `element_*`s are for modifying *aesthetics*. 
...
`element_style` has three arguments: the columns in the final table to be aligned,
...
When the `col_style_plan()` is applied to the data, the most recent alignment and width is applied to the data, so be careful when adding new `element_style`'s, as the output may change more than anticipated if the new element is not specific enough. 
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    element_style(align = "left", col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    element_style(align = c("."), col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    element_style(align = c("."), col = `p-value`),
    element_style(align = "left", col = trt1),
    element_style(align = "right", col = trt2))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    element_style(width = "300px", col = label),
    element_style(align = c("."), width = "100px", col = `p-value`),
    element_style(align = "left", width = "250px", col = trt1),
    element_style(align = "right", width = "250px", col = trt2))
  ) %>% 
  print_to_gt(dat)
