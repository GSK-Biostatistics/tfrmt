...
`col_style_structure` has three arguments: the columns in the final table to be aligned, the alignment desired, the widths to apply to those columns. In each element, only one of the who (alignment, width), must be applied. Optionally both can be assigned too.
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    col_style_structure(align = "left", col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    col_style_structure(align = c("."), col = `p-value`))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    col_style_structure(align = c("."), col = `p-value`),
    col_style_structure(align = c(".", ",", " "), col = starts_with("trt"))
    )
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    ...
    col_style_structure(align = "right", col = trt2))
  ) %>% 
  print_to_gt(dat)
```
...
tfrmt(
  ...
  col_style_plan =  col_style_plan(
    col_style_structure(width = "10", col = label),
    col_style_structure(align = c("."), width = "10", col = `p-value`),
    col_style_structure(align = "left", width = "5", col = trt1),
    col_style_structure(align = "right", width = "5", col = trt2))
  ) %>% 
  print_to_gt(dat)
