...
create_tbl_demog_data <- function() {
    ...
    age_2 <- adsl_ %>%
        sum_subgrp(AGEGR1, AGEGR1N, include.n = FALSE, header_n = header_n)
    ...
    # Categorical
    durdis_2 <- adsl_ %>%
        sum_subgrp(DURDSGR1, DURDSGR1N, include.n = FALSE, header_n = header_n)
    ...
    # Categorical
    bmi_2 <- adsl_ %>%
        sum_subgrp(BMIBLGR1, BMIBLGR1N, include.n = FALSE, header_n = header_n)
    ...
    final_0 <- final %>%
        ...
        mutate(
            data = data %>%
                mutate(ord2 = fct_inorder(rowlbl2) %>% as.numeric) %>%
                list
        ) %>%
        unnest(data) %>%
        pivot_longer(
            -c(rowlbl1, rowlbl2, grp, param, ord1, ord2),
            names_to = "column",
            values_to = "value"
        ) %>%
        mutate(
            param = ifelse(column == "p", "p", param),
