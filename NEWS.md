...
* Harmonised condition handling: all errors, warnings, and messages now use namespaced {cli} calls (`cli::cli_abort()`, `cli::cli_warn()`, `cli::cli_inform()`) instead of a mixture of base R (`stop()`, `warning()`, `message()`) and {rlang} (`abort()`, `warn()`, `inform()`) (#748).
